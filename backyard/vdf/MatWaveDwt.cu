...
// Sam: CUDA test code
__global__ void testcuda( int* a, int* b, int* c)
{
    size_t idx = threadIdx.x + blockIdx.x * blockDim.x;
    c[idx] = a[idx] + b[idx];
}
// Stencil kernel for forward xform, version 1
__global__ void kernel_conv( double* sigIn, size_t sigInLen,
                        ...
                        size_t xlstart, size_t xhstart )
{
    ...
    if( yi < sigInLen ) {
        ...
        for( int k = filterLen - 1; k >= 0; k-- ) {
			cA[yi2] += low_filter[k] * sigIn[xl];
			cD[yi2] += high_filter[k] * sigIn[xh];
			xl++;
			xh++;
        }
    }
...
void cuda_forward_xform (
    ...
    double *cA, double *cD, size_t cALen, size_t cDLen, 
    bool oddlow, bool oddhigh) 
{
	size_t xlstart = oddlow ? 1 : 0;
	size_t xhstart = oddhigh ? 1 : 0;
    ...
    if( cudaSuccess != cudaMalloc( (void**) &d_low_filter, filterLen * sizeof(double) )) {
        cerr <<  "cudaMalloc fail: d_low_filter" << endl;
        exit(1);
    }
    if( cudaSuccess != cudaMalloc( (void**) &d_high_filter, filterLen * sizeof(double) )) {
        cerr <<  "cudaMalloc fail: d_high_filter" << endl;
        exit(1);
    }
    if( cudaSuccess != cudaMalloc( (void**) &d_cA, cALen * sizeof(double) )) {
        cerr <<  "cudaMalloc fail: d_cA" << endl;
        exit(1);
    }
    if( cudaSuccess != cudaMalloc( (void**) &d_cD, cDLen * sizeof(double) )) {
        cerr <<  "cudaMalloc fail: d_cD" << endl;
        exit(1);
    }
    cudaMemcpy( d_sigIn, sigIn, sigInExtendedLen * sizeof(double), cudaMemcpyHostToDevice );
...
//    kernel_conv<<< (nBlock, 1, 1), (nThread, 1, 1) >>>( d_sigIn, sigInLen, 
    ...
    cudaMemcpy( cD, d_cD, cDLen * sizeof(double), cudaMemcpyDeviceToHost );
    cudaFree( d_sigIn );
...
// this version outputs debug info, and compares result
void cuda_forward_xform_debug (
	const double *sigIn, size_t sigInLen, size_t sigInExtendedLen,
	const double *low_filter, const double *high_filter, int filterLen, 
    double *cA, double *cD, size_t cALen, size_t cDLen, 
    bool oddlow, bool oddhigh) 
...
cerr << "xform debug info: " << endl;
cerr << "sigInLen, sigInExtendedLen, filterLen, cALen, cDLen: " 
     << sigInLen << ",  " << sigInExtendedLen << ",  " << filterLen 
     << ",  " << cALen << ",  " << cDLen << endl; 
	...
	for (size_t yi = 0; yi < sigInLen; yi += 2) {           // 0, 2, 4, 6 ...
        yi2 = yi/2;                                  // 0, 1, 2, 3 ...
        cA[ yi2 ] = cD[ yi2 ] = 0.0;
    ...
    double *tmp_cD = new double[ cDLen ];
    for( size_t i = 0; i < cmin; i++ ) 
        tmp_cA[i] = tmp_cD[i] = 0.0;
    cudaMemcpy( tmp_cA, d_cA, cALen * sizeof(double), cudaMemcpyDeviceToHost );
    cudaMemcpy( tmp_cD, d_cD, cDLen * sizeof(double), cudaMemcpyDeviceToHost );
    cudaFree( d_sigIn );
    cudaFree( d_low_filter );
    cudaFree( d_high_filter );
    cudaFree( d_cA );
    ...
    for( size_t i = 0; i < cmin; i++ ) {
        if( abs(tmp_cA[i] - cA[i]) > epsilon )
            cerr << "\tcA: " << cA[i] << ",  " << tmp_cA[i] << endl;
        if( abs(tmp_cD[i] - cD[i]) > epsilon )
            cerr << "\tcD: " << cD[i] << ",  " << tmp_cD[i] << endl;
    }
    delete[] tmp_cA;
    delete[] tmp_cD;
}
